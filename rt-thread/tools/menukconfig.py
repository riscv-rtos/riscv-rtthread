...
def defconfig(RTT_ROOT, configfn):
    import defconfig
    ...
    configfn = os.path.join(os.getenv('RV_RTT_ROOT'), 'boards', 'configs', configfn)
    if not os.path.isfile(configfn):
        print("config file %s not exist!" % configfn)
        return False
    
    sys.argv = ['defconfig', '--kconfig', 'Kconfig', configfn]
    defconfig.main()

    # silent mode, force to make rtconfig.h
    mk_rtconfig(fn)

def savedefconfig(RTT_ROOT, configfn):
    ...
    sys.argv = ['savedefconfig', '--kconfig', 'Kconfig', '--out', configfn]
    savedefconfig.main()
