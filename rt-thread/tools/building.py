...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    if rtconfig.PLATFORM in ['gcc']:
        if not GetOption('defconfig') and not GetOption('menuconfig'):
            from install_tools import install

            print("****************** build information ******************")
            print(f"CHIP_FAMILY: {os.getenv('CHIP_FAMILY')}")
            print(f"BOARD_NAME: {os.getenv('BOARD_NAME')}")
            print(f"CHIP_NAME: {os.getenv('CHIP_NAME')}")
            print("*******************************************************")

            tools_path = os.path.join(os.getenv('RTT_ROOT'), 'chip', os.getenv('CHIP_FAMILY'), os.getenv('CHIP_NAME'))
            if install(os.path.join(tools_path, 'tools.json')):
    ...
    configfn = GetOption('defconfig')
    if configfn:
        from menukconfig import defconfig
        defconfig(Rtt_Root, configfn)
        exit(0)

    configfn = GetOption('savedefconfig')
    if configfn:
        from menukconfig import savedefconfig
        savedefconfig(Rtt_Root, configfn)
        exit(0)
